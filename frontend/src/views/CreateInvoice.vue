...
export default class CreateInvoice extends Vue {
  ...
  downloadPdf() {
    ...
    let width = this.$refs.invoice.clientWidth - 100;
    let height = this.$refs.invoice.clientHeight - 100;
    html2canvas(invoice).then(canvas => {
      let image = canvas.toDataURL("images/png");
      pdf.addImage(image, "PNG", 0, 0, width * 0.55, height * 0.55);
      pdf.save(`invoice-${this.selectedCustomer.firstName} ${this.selectedCustomer.lastName}`);
    });
  }
